...
function FormRow() {
  return (
    <React.Fragment>
      <Grid item xs={4}>
        <Typography
          sx={{ fontSize: "24px", fontWeight: "600", color: "#828282" }}
        >
...
export default function JobTitle(props) {
  ...
  useEffect(() => {
    ...
    gqlquery(QUERY_GETAPPLICANTSLISTBYJOB, null)
      .then((res) => res.json())
      .then((data) => setApplicantsList(data?.data?.getApplicantsListByJob));
  }, []);

  console.log("from job title", applicantsList)
    ...
    <Container maxWidth="md" sx={{ mb: 8 }}>
      <Box style={{ marginTop: "15px", marginBottom: "30px" }}>
        ...
        >
          ...
          <Typography color="text.secondary">{vacancyTitle}</Typography>
        </Breadcrumbs>
        <Typography
          variant="p"
              ...
              ?.map((row, index) => (
                ...
                >
                  ...
                  <TableCell
                    ...
                    style={{ padding: "0px", margin: "0px", fontWeight: 600 }}
                  >
                    <Link to={`/applicants-detail/${row?.userID}`} state={{ jobTitle: vacancyTitle, link: location?.pathname, userID: row?.userID }} > {row?.name} </Link>
                  </TableCell>
                  <TableCell
                    align="left"
                    style={{ padding: "0px", margin: "0px", fontWeight: 600 }}
